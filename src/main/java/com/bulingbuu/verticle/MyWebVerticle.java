...
import com.bulingbuu.bean.Whisky;
...
public class MyWebVerticle extends AbstractVerticle {
    ...
    public void start(Future<Void> fut) {
        ...
        JsonObject mySQLClientConfig = new JsonObject()
                .put("host", "localhost")
                .put("port", 3306)
                .put("database", "whiskies")
                .put("maxPoolSize", 30)
                .put("username", "root")
                .put("password", "19950719l");
    ...
    private void createSomeData(AsyncResult<SQLConnection> result, Handler<AsyncResult<Void>> next, Future<Void> fut) {
        if (result.failed()) {
            ...
            connection.execute(
                    "CREATE TABLE IF NOT EXISTS Whisky (id int(4) primary key not null auto_increment, name varchar(100), origin varchar" +
                            "(100))",
                    ar -> {
                        if (ar.failed()) {
                            fut.fail(ar.cause());
